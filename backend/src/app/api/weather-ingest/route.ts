...
import { CwaApiResponse, CwaApiStation, WeatherSnapshot } from "@/lib/weather-types";
...
async function ingestWeatherData() {
  ...
  console.log(`[Weather Ingest] 成功轉換 ${snapshot.stations.length} 筆測站資料，時間戳記: ${snapshot.timestamp.toISOString()}`);
  ...
  // 檢查具有相同時間戳記的文件是否已存在
  const existingDoc = await collection.findOne({ timestamp: snapshot.timestamp });
...
export async function GET() {
  ...
  try {
    ...
    return NextResponse.json(
      { success: true, message: result.message, insertedId: result.insertedId, insertedCount: result.insertedCount },
